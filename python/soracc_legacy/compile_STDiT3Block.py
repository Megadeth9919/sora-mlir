...
def main():
    ...
    B, T, S = 2, 16, 144
    cond_size = 54
    x_tensor = Tensor(name="x_tensor", shape=Shape(B, T * S, 1152), dtype=DataType.float16)
    y_tensor = Tensor(name="y_tensor", shape=Shape(1, cond_size, 1152), dtype=DataType.float16)
    t_tensor = Tensor(name="t", shape=Shape(1, B, 1152 * 6), dtype=DataType.float16)

    x_tensor_data = np.random.randn(B, T * S, 1152).astype(np.float16)
    y_tensor_data = np.random.randn(1, cond_size, 1152).astype(np.int8)
    t_tensor_data = np.random.randn(1, B, 1152 * 6).astype(np.float16)

    x_tensor.set_data(x_tensor_data)
    y_tensor.set_data(y_tensor_data)
    t_tensor.set_data(t_tensor_data)

    g = get_single_STDiT3Block(x_tensor=x_tensor, y_tensor=y_tensor, t_tensor=t_tensor, T=T, S=S,)
    g.complete()
    print(json.dumps(g.to_json(), indent=2))
    inputs = [x_tensor, y_tensor, t_tensor, ]
    index = 0
    ...
    dump_information(g, "cases/ditblock",op_name="ditblock")
